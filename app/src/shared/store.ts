...
export class DocStore {
  ...
  // ---- pre-run snapshot undo (the one-Cmd-Z mechanism, §5.1) ----
  /** Deep-clone the live state so the snapshot is never aliased into the map. */
  snapshot(): { nodes: Map<NodeId, Node>; version: DocVersion; idSeq: number } {
    return {
  ...
  /** Restore a snapshot, deep-cloning back so the snapshot stays unaliased. */
  restore(s: { nodes: Map<NodeId, Node>; version: DocVersion; idSeq: number }) {
    this.nodes = structuredClone(s.nodes);
